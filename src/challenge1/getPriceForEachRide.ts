...
export function getPriceForEachRide(routes: Routes[], rideService: RideService[], bookedRide: Ride[]) {
    ...
    return bookedRide.map((bookedRide: Ride) => {
        ...
        const rideServiceBeingUsed = rideService.find((service: RideService) => service.rideservice_id === rideSercviceId);
        const rideServiceRate = parseInt(rideServiceBeingUsed?.priceperkm as string);
